...
[ApiController]
[Route("[controller]")]
public class BillingController(IAccountService service, ILogger<BillingController> logger, IMapper mapper)
   : ControllerBase
{
    ...
    [HttpPut()]
    public async Task<IActionResult> Change([FromBody] decimal ammount,
        [FromHeader(Name = "X-Auth-Request-Preferred-Username")] string clientID)
    {
        ...
        return Ok(await service.ChangeBalance(changeDto));
    }
